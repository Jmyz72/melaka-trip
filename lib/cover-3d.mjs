...
function buildFrontTexture() {
  ...
  for (let i = 0; i <= coastN; i++) {
    const t = i / coastN;
    // Base diagonal from (0.32, 0) to (0.18, 1) of map width
    const baseX = 0.34 - 0.16 * t;
  ...
  for (let y = mapY + 8; y < mapY + mapH; y += 14) {
    ...
    for (let x = mapX + 4; x < mapX + 0.32 * mapW; x += 8) {
      ...
      const t = (y - mapY) / mapH;
      const coastX = mapX + (0.34 - 0.16 * t + Math.sin(t * Math.PI * 6) * 0.018 - Math.exp(-Math.pow((t - 0.78) * 6, 2)) * 0.08) * mapW;
