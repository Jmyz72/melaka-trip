...
function buildFrontTexture() {
  ...
  // ── Strait of Malacca (sea) on the west side ──
  // Coastline is a wavy diagonal from top-left running down to bottom-left,
  // bulging into the map at lower-middle (where Melaka city is)
  const seaPath = [];
  const coastN = 60;
  for (let i = 0; i <= coastN; i++) {
    const t = i / coastN;
    // Base diagonal from (0.24, 0) to (0.08, 1) of map width.
    // Tuned so the Seremban marker (lng 101.94) and pin 1 (lng 101.91)
    // sit east of the coastline rather than appearing to float in the sea.
    const baseX = 0.24 - 0.16 * t;
    // Local wobble + a bulge near the bottom third
    const wobble = Math.sin(t * Math.PI * 6) * 0.018 + Math.sin(t * Math.PI * 13) * 0.008;
    const bulge  = -Math.exp(-Math.pow((t - 0.78) * 6, 2)) * 0.08; // sea pushes east at Melaka
    seaPath.push({
      x: mapX + (baseX + wobble + bulge) * mapW,
      y: mapY + t * mapH
    });
  }
  // Sea fill: trapezoid bounded by left edge + coastline
  ctx.beginPath();
  ctx.moveTo(mapX, mapY);
  for (const p of seaPath) ctx.lineTo(p.x, p.y);
  ctx.lineTo(mapX, mapY + mapH);
  ctx.closePath();
  const seaGrad = ctx.createLinearGradient(mapX, mapY, mapX + 0.35 * mapW, mapY);
  ...
  for (let y = mapY + 8; y < mapY + mapH; y += 14) {
    ctx.beginPath();
    let started = false;
    for (let x = mapX + 4; x < mapX + 0.32 * mapW; x += 8) {
      // Don't draw past the coastline
      const t = (y - mapY) / mapH;
      const coastX = mapX + (0.24 - 0.16 * t + Math.sin(t * Math.PI * 6) * 0.018 - Math.exp(-Math.pow((t - 0.78) * 6, 2)) * 0.08) * mapW;
      if (x > coastX - 10) break;
      const offset = Math.sin((x + y) * 0.18) * 1.6;
      if (!started) { ctx.moveTo(x, y + offset); started = true; }
  ...
  ctx.lineWidth = 1.4;
  for (let i = 0; i < seaPath.length; i++) {
    const p = seaPath[i];
  ...
  // ── Melaka River meandering through land ──
  ctx.beginPath();
  ctx.strokeStyle = "rgba(80, 120, 145, 0.55)";
  ctx.lineWidth = 1.8;
  const riverStartT = 0.82, riverEndT = 0.62;
  const samples = 40;
  for (let i = 0; i <= samples; i++) {
    const t = riverStartT + (riverEndT - riverStartT) * (i / samples);
    const coastX = mapX + (0.24 - 0.16 * t + Math.sin(t * Math.PI * 6) * 0.018 - Math.exp(-Math.pow((t - 0.78) * 6, 2)) * 0.08) * mapW;
    const advance = (i / samples);
    const rx = coastX + advance * mapW * 0.45 + Math.sin(advance * Math.PI * 4) * 12;
    const ry = mapY + (t + Math.sin(advance * Math.PI * 3) * 0.02) * mapH;
  ...
  // ── Trip polyline + pins ──
  const pinPos = STOPS.map(s => ({ ...projectLatLng(s.lat, s.lng, mapX, mapY, mapW, mapH), n: s.n }));
