...
const STOPS = [
  ...
  { lat: 2.19439, lng: 102.24400, n: 9 }, // Duck Noodle
  { lat: 2.19706, lng: 102.23151, n: 10 }, // The Whole Gang
  { lat: 2.18855, lng: 102.26174, n: 11 }, // Restaurant Baba Ang
  { lat: 2.19534, lng: 102.24799, n: 12 }, // Jonker Walk
  { lat: 2.18979, lng: 102.21982, n: 13 }, // Seaside
  { lat: 2.26325, lng: 102.24213, n: 14 }, // Hock Chin Seafood
  { lat: 2.19665, lng: 102.23115, n: 15 }, // Lim Jiu
  { lat: 2.18473, lng: 102.26013, n: 16 }, // La Mille Bakers
  { lat: 2.19464, lng: 102.25308, n: 17 }, // Something Bakery
];
...
function buildFrontTexture() {
  ...
  // ── Trip polyline + pins ──
  // Co-located moments (the Airbnb base, revisited six times) would stack into
  // one illegible blob. Group stops that project to nearly the same pixel and
  // draw a single "home base" marker (house glyph + ×count); the dashed route
  // still threads back through that anchor between excursions.
  const CLUSTER_RADIUS_PX = 18;
  const projected = STOPS.map(s => {
    const p = projectLatLng(s.lat, s.lng, mapX, mapY, mapW, mapH);
    return { x: p.x, y: p.y, n: s.n };
  });
  const groups = [];
  for (const p of projected) {
    let g = null;
    for (const grp of groups) {
      const dx = grp.x - p.x, dy = grp.y - p.y;
      if (dx * dx + dy * dy < CLUSTER_RADIUS_PX * CLUSTER_RADIUS_PX) { g = grp; break; }
    }
    if (g) g.members.push(p);
    else groups.push({ x: p.x, y: p.y, members: [p] });
  }

  // Route threads through each group's anchor in trip order, dropping the
  // consecutive repeats (a stay at one place is a point, not a segment).
  const groupOf = new Map();
  groups.forEach(g => g.members.forEach(m => groupOf.set(m, g)));
  const route = [];
  for (const p of projected) {
    const g = groupOf.get(p);
    const last = route[route.length - 1];
    if (!last || last.x !== g.x || last.y !== g.y) route.push({ x: g.x, y: g.y });
  }
  ...
  // Pin markers — single stops keep their number; a grouped base becomes one
  // larger home pin with a house glyph and a ×count chip.
  groups.forEach(g => {
    const isBase = g.members.length > 1;
    const r = isBase ? 16 : 13;
    // Halo
    ctx.beginPath();
    ctx.arc(g.x, g.y, r + 5, 0, Math.PI * 2);
    ...
    if (isBase) {
      // Little house glyph in paper color
      ctx.strokeStyle = "#f4ead5";
      ctx.lineWidth = 1.8;
      ctx.lineJoin = "round";
      ctx.lineCap = "round";
      const u = r * 0.5;
      ctx.beginPath();
      ctx.moveTo(g.x - u, g.y);                 // roof left
      ctx.lineTo(g.x, g.y - u);                 // apex
      ctx.lineTo(g.x + u, g.y);                 // roof right
      ctx.moveTo(g.x - u * 0.78, g.y - u * 0.16); // walls
      ctx.lineTo(g.x - u * 0.78, g.y + u * 0.85);
      ctx.lineTo(g.x + u * 0.78, g.y + u * 0.85);
      ctx.lineTo(g.x + u * 0.78, g.y - u * 0.16);
      ctx.stroke();
      // Count chip, upper-right
      const bx = g.x + r * 0.92, by = g.y - r * 0.92;
      ctx.beginPath();
      ctx.arc(bx, by, 9, 0, Math.PI * 2);
      ctx.fillStyle = "#f4ead5";
      ctx.fill();
      ctx.strokeStyle = "#9e2a1e";
      ctx.lineWidth = 1.5;
      ctx.stroke();
      ...
      ctx.textAlign = "center";
      ctx.textBaseline = "middle";
      ctx.fillText(String(g.members.length), bx, by + 0.5);
    } else {
      // Number
      ctx.fillStyle = "#f4ead5";
      ...
      ctx.textAlign = "center";
      ctx.textBaseline = "middle";
      ctx.fillText(String(g.members[0].n), g.x, g.y + 0.5);
    }
  });
