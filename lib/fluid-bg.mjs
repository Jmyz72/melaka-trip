...
const STEP_SRC = `
  ...
  void main() {
    ...
    float blob = exp(-d * d / 0.006);
    float speed = clamp(length(uMouseVel) * 50.0, 0.0, 4.0);
    float ink   = blob * (0.18 + speed * 0.20) * uMouseStrength;
    // Color of the ink: indigo, slightly cooler when stationary
    vec3 inkColor = mix(vec3(0.11, 0.17, 0.31), vec3(0.78, 0.22, 0.17), 0.0);
    col.rgb += inkColor * ink;
    col.a   += ink * 0.6;

    // Subtle ambient: a quiet wash of ink from the noise field so the screen
    // is never fully empty
    float ambient = fbm(uv * 1.8 + uTime * 0.015) * 0.0008;
    col.rgb += vec3(0.11, 0.17, 0.31) * ambient;
    col.a   += ambient * 0.5;
...
const DISPLAY_SRC = `
  ...
  void main() {
    ...
    // Subtle paper grain
    float grain = (hash(floor(vUV * 1400.0)) - 0.5) * 0.02;
    paper += grain;
    // Ink over paper with multiply-ish blend so it darkens
    vec3 col = paper * (1.0 - ink.a * 0.55) + ink.rgb * ink.a * 0.85;
    // Slight vignette
...
export function mountFluidBG(canvas) {
  ...
  function ambientDrift(now) {
    ...
    driftAngle += 0.012;
    mouseX = 0.5 + Math.cos(driftAngle) * 0.3;
    mouseY = 0.5 + Math.sin(driftAngle * 1.3) * 0.3;
