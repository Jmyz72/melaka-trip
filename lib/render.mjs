...
function ratingHTML(stopId) {
  ...
  return `
    <div class="rating is-unrated">
      <div class="rating-label">tap a star</div>
...
export function applyRatingForStop(stopId, summary) {
  ...
  // Once this friend has rated, drop the "tap a star" invitation.
  const ratingEl = root.querySelector(".rating");
  if (ratingEl) {
    const rated = my > 0;
    ratingEl.classList.toggle("is-unrated", !rated);
    const labelEl = ratingEl.querySelector(".rating-label");
    if (labelEl) labelEl.textContent = rated ? "your rating" : "tap a star";
  }
