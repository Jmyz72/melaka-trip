...
export function mountSideTimeline(stops, scrollEl, root) {
  ...
  // On phone the rail overlaps content, so it only shows while the user is
  // actively scrolling. Tap `is-scrolling` on every scroll event; clear it a
  // short delay after motion stops. (No-op on desktop where CSS keeps the
  // rail visible regardless.)
